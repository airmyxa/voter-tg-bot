...
impl HandlerTr<MessageRequest, Dependencies> for Handler {
    async fn handle(self, request: MessageRequest, dependencies: Dependencies)
              -> HandlerResult {
        info!("Start handling help request: {}", request.message.text().unwrap());
...
impl Handler {
    async fn send_help_message(self, request: MessageRequest, dependencies: Dependencies)
        -> HandlerResult {
        request.bot.send_message(
            request.message.chat.id, Command::descriptions().to_string()).await?;
        Ok(())
    }
}
